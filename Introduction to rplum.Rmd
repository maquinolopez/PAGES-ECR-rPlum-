...
In this case we used a core which only has 210Pb measurements and we will have the last 6 data points to be used exclusively for supported 201Pb estimation (constant). If a proxy for supported 210Pb is avalable the info column can be added as the 9th column (after the mesurement and standar deviation of the proxy for supported 210Pb).
...
To run a core which has 210Pb and 14C (or other dates) you can add a extra file into the core folder. This new file can be call anything but we recomment to keep the convention of the `core_name-C.csv`. 
